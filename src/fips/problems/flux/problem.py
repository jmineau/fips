...
class FluxProblem(InverseProblem):
    ...
    constant : Vector or None
        Optional constant term added to the forward model (e.g., background).
    estimator : Estimator
        The fitted estimator after solving the problem.
    state_index : pd.Index
        Index of the state space.
    obs_index : pd.Index
        Index of the observation space.
    n_state : int
        Number of state variables.
    n_obs : int
        Number of observations.
    posterior : Vector
        Posterior state estimate after solving.
    posterior_error : CovarianceMatrix
        Posterior error covariance after solving.
    posterior_obs : Vector
        Modeled observations using the posterior state.
    prior_obs : Vector
        Modeled observations using the prior state.
    kalman_gain : Matrix
        Kalman gain matrix (K) after solving.
    averaging_kernel : Matrix
        Averaging kernel matrix (A) after solving.
    concentrations : pd.Series
        Observed concentrations.
    enhancement : pd.Series
        Background-subtracted observation vector.
    prior_fluxes : pd.Series
        Prior flux inventory.
    jacobian : pd.DataFrame
        Forward operator (Jacobian) mapping fluxes to concentrations.
    prior_flux_error : pd.DataFrame
        Inventory error covariance matrix.
    concentration_error : pd.DataFrame
        Model-data mismatch covariance matrix for observations.
    background : pd.Series or None
        Background concentration.
    posterior_fluxes : pd.Series
        Posterior flux estimates after inversion.
    posterior_flux_error : pd.DataFrame
        Posterior flux error covariance matrix after inversion.
    prior_concentrations : pd.Series
        Modeled concentrations from prior fluxes.
    posterior_concentrations : pd.Series
        Modeled concentrations from posterior fluxes.
    plot : FluxPlotter
    ...
    solve(estimator='bayesian', **kwargs)
    ...
    def solve(self, estimator: str | type[Estimator] = "bayesian", **kwargs) -> Self:
        ...
        Parameters
        ----------
        estimator : str or type[Estimator], default 'bayesian'
            Estimator to use for solving. Can be a registered estimator name or Estimator subclass.
        **kwargs
        ...
        -------
        Self
            The FluxProblem instance with the fitted estimator.
        """
