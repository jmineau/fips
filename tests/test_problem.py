...
class TestInverseProblemCreation:
    ...
    def test_inverse_problem_basic_creation(self, simple_prior, simple_obs, 
    ...
    def test_inverse_problem_with_vector_objects(self):
        ...
        block_prior = Block('state', pd.Series([1.0, 2.0], index=['x', 'y']))
        ...
        H = pd.DataFrame([[1, 0.5], [0.5, 1]], 
                        index=['obs_0', 'obs_1'],
                        columns=prior_idx)
        
        S_0 = pd.DataFrame(np.eye(2), index=prior_idx, columns=prior_idx)
        S_z = pd.DataFrame(np.eye(2), index=['obs_0', 'obs_1'], columns=['obs_0', 'obs_1'])
    ...
    def test_inverse_problem_stores_matrices(self, simple_prior, simple_obs,
                                            simple_forward_operator,
                                            simple_prior_error,
                                            simple_modeldata_mismatch):
    ...
    def test_inverse_problem_stores_vectors(self, simple_prior, simple_obs,
                                           simple_forward_operator,
                                           simple_prior_error,
                                           simple_modeldata_mismatch):
...
class TestInverseProblemProperties:
    ...
    def test_prior_property(self, simple_prior, simple_obs,
                           simple_forward_operator,
                           simple_prior_error,
                           simple_modeldata_mismatch):
    ...
    def test_obs_property(self, simple_prior, simple_obs,
                         simple_forward_operator,
                         simple_prior_error,
                         simple_modeldata_mismatch):
    ...
    def test_forward_operator_property(self, simple_prior, simple_obs,
                                      simple_forward_operator,
                                      simple_prior_error,
                                      simple_modeldata_mismatch):
    ...
    def test_prior_error_property(self, simple_prior, simple_obs,
                                 simple_forward_operator,
                                 simple_prior_error,
                                 simple_modeldata_mismatch):
    ...
    def test_modeldata_mismatch_property(self, simple_prior, simple_obs,
                                        simple_forward_operator,
                                        simple_prior_error,
                                        simple_modeldata_mismatch):
...
class TestInverseProblemMatrixHandling:
    ...
    def test_matrix_wrapping_covariance(self, simple_prior, simple_obs,
                                       simple_forward_operator,
                                       simple_prior_error,
                                       simple_modeldata_mismatch):
    ...
    def test_matrix_wrapping_forward_operator(self, simple_prior, simple_obs,
                                             simple_forward_operator,
                                             simple_prior_error,
                                             simple_modeldata_mismatch):
...
class TestInverseProblemIndexAlignment:
    ...
    def test_mismatched_prior_indices_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert any("Partial overlap" in msg or "No overlap" in msg 
    ...
    def test_matrix_reindexing_fills_zeros(self):
        ...
        H = pd.DataFrame([[1, 0.5, 0], [0.5, 1, 0.5]], 
                        index=['o1', 'o2'],
                        columns=['a', 'b', 'c'])
...
class TestInverseProblemFloatPrecision:
    ...
    def test_float_precision_rounding(self):
        ...
        prior = pd.Series([1.0, 2.0], 
                         index=[1.123456, 2.654321],
                         name='prior')
        obs = pd.Series([1.0, 2.0], 
                       index=[3.111111, 4.222222],
                       name='obs')
        
        H = pd.DataFrame(np.eye(2),
                        index=[3.111111, 4.222222],
                        columns=[1.123456, 2.654321])
        
        S_a = pd.DataFrame(np.eye(2), 
                          index=[1.123456, 2.654321],
                          columns=[1.123456, 2.654321])
        S_z = pd.DataFrame(np.eye(2),
                          index=[3.111111, 4.222222],
                          columns=[3.111111, 4.222222])
...
class TestInverseProblemDimensions:
    ...
    def test_rectangular_problem(self):
        ...
        H = pd.DataFrame(np.ones((10, 5)),
                        index=[f'o{i}' for i in range(10)],
                        columns=[f's{i}' for i in range(5)])
        
        S_a = pd.DataFrame(np.eye(5), index=[f's{i}' for i in range(5)],
                          columns=[f's{i}' for i in range(5)])
        S_z = pd.DataFrame(np.eye(10), index=[f'o{i}' for i in range(10)],
    ...
    def test_square_problem(self):
        ...
        H = pd.DataFrame(np.eye(n),
                        index=[f'o{i}' for i in range(n)],
                        columns=[f's{i}' for i in range(n)])
        
        S_a = pd.DataFrame(np.eye(n), index=[f's{i}' for i in range(n)],
                          columns=[f's{i}' for i in range(n)])
        S_z = pd.DataFrame(np.eye(n), index=[f'o{i}' for i in range(n)],
    ...
    def test_underdetermined_problem(self):
        ...
        H = pd.DataFrame(np.ones((5, 10)),
                        index=[f'o{i}' for i in range(5)],
                        columns=[f's{i}' for i in range(10)])
        
        S_a = pd.DataFrame(np.eye(10), index=[f's{i}' for i in range(10)],
                          columns=[f's{i}' for i in range(10)])
        S_z = pd.DataFrame(np.eye(5), index=[f'o{i}' for i in range(5)],
...
class TestInverseProblemInitializerNone:
    ...
    def test_estimator_initially_none(self, simple_prior, simple_obs,
                                     simple_forward_operator,
                                     simple_prior_error,
                                     simple_modeldata_mismatch):
