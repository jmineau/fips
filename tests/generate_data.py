...
def generate_test_data(
    ...
    truth_vals += 10.0
    truth_state = pd.Series(truth_vals, index=idx_state, name="state_block")
    ...
    H_df.columns = promote_index(H_df.columns, "state_block", "block")

    # 5. Covariances (CovarianceMatrix)
    # Temporary vectors for builder logic
    prior_vec = Vector(name="prior", blocks=[Block(prior_state)])
    # Placeholder obs series just for structure
    obs_placeholder = pd.Series(np.zeros(n_obs_tot), index=idx_obs, name="obs_block")
    obs_vec = Vector(name="obs", blocks=[Block(obs_placeholder)])
    ...
    S_prior = CovarianceMatrix.from_vector(prior_vec)
    S_prior.set_block(
        "state_block", sigma=prior_sigma, kernel=nd_decay(1.0, correlation_len)
    )

    # Build Obs Error (S_z)
    S_obs = CovarianceMatrix.from_vector(obs_vec)
    S_obs.set_block("obs_block", sigma=obs_sigma)
    ...
    obs_state = pd.Series(y_obs_vals, index=idx_obs, name="obs_block")
